...
let ntests=15000,seed=0,Ωₘ=2000
    let ℓ=2,R=4,clusterFun=Ζ, amplitudeFun = imag1s
        let pertFun=𝔉ᵣ(Ωₘ,seed)
            let ϵR = 10.0.^LinRange(-14,-3,150), ΩR = NrangeLog(1.2,3,150), ΔR = 10.0.^LinRange(-4,-2,150)
                let res = mapreduce(transpose,vcat,
                    ...
                    let clr = map(cc,res[:,2]), sh = map(ss,res[:,2]),idxsSucc = findall(res[:,2].==1),idxsFail=findall(res[:,2].==0)
                        ...
                        #res[:,2]
                        plot!(t->(2*ℓ-1)*t+2,w=2,l=:dash,lc=:black,
                            lab="\$ SRF^{2p-1}\$")
                        ylims!(3,12)
                        xlims!(1,3)
                        title!("p=$(ℓ),d=$(R), \$ {\\bf S_1} \$")
                        #savefig(p,"../figures/phase_tran_4.pdf")
                    end
                end
            end
        end
